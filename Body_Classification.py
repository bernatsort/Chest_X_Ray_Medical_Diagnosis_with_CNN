...
def classification_VGG(main_dir: str, csv_dir: str):
    ...
    for i in range(len(image_names)):
        for row in tags_df.index:
            if tags_df.loc[row, "SOPInstanceUID"] in image_names[i]:
                new_label = tags_df.loc[row, "Target"].split(' ')[0]
...
def classification_VGG_bin(main_dir: str, csv_dir: str):
    ...
    for i in range(len(image_names)):
        for row in tags_df.index:
            if tags_df.loc[row, "SOPInstanceUID"] in image_names[i]:
                ...
                if int(new_label) == 3 or int(new_label) == 20:
                    new_label = 0
                elif int(new_label) == 4 or int(new_label) == 14:
                    new_label = 1
                else:
                    new_label = 1
...
def descriptive_analysis(tags_df):
    
    print(tags_df['Target'].value_counts())
